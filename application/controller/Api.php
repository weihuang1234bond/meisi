...
class Api extends Controller
{
    ...
    public function login(Request $request)
    {
        if (get_config('verification_code_on')) {
            ...
            if (!captcha_check($verifyCode,session('captcha_id'))) die(json_encode(['statusCode' => 4005, 'error' => '数据验证失败:验证码错误']));
        }
           ...
           switch ($userType) {
            ...
            if ($result !== true) die(json_encode(['statusCode' => 4005, 'error' => '数据验证失败:' . $result]));
             if (get_config('verification_code_on')) {
                  ...
                  if (!captcha_check($verifyCode,session('captcha_id'))) die(json_encode(['statusCode' => 4005, 'error' => '数据验证失败:验证码错误5']));
              }
...
//                    邮箱发送验证码处理
    ...
    public function getCaptcha($id=0)
    {
        $id = time();
        session('captcha_id',$id);
        //var_dump(session('captcha_id'));exit;
        return create_captcha($id);
    }
        ...
        if (empty(session('last_send_member_mobile_time_' . $sendType)) || session('last_send_member_mobile_time_' . $sendType) < time() - 30) {
            ...
            switch ($sendType) {
                case 'checkOld':
                    ...
                    $smsdata = array(
                        'tel' => $rs['tel'],
                        'msg' => '【奇米吧】亲爱的用户，您好 !您正在进行手机更换操作，安全验证码为： ' . $checkMailCode
                    );
                    break;
                ...
                case 'authNew':
                    ...
                    $smsdata = array(
                        'tel' => $toMobile,
                        'msg' => '【奇米吧】亲爱的用户，您好 !您正在进行手机绑定操作，安全验证码为： ' . $checkMailCode
                    );
                    break;
            }
            if ($status == 1) {
...
//                   手机发送验证码处理
                ...
                $smsdata = array(
                    'tel' => $content,
                    'msg' => '【奇米吧】亲爱的用户，您好 !您正在进行找回密码操作，安全验证码为： ' . $code
                );
                $sms = new Sms();
                $sms->send($smsdata);
                break;
    ...
    public function moreData(Request $request)
    {
        ...
        $area_id = $request->post('area_id/d', '0');//new add 4/22
        ...
        if (!empty($area_id)) {
            $where .= " and FIND_IN_SET( $area_id, {$pre}{$type}.area_id)";
        }
