...
class Novel extends BaseController
{
    ...
    public function lists(Request $request)
    {
        ...
        $area_list  = custom_class('3'); //2019/4/22 区域 thirteen 1视频 2为图片 3为资讯
...
//            $where .= " and tag = $tag_id";
        ...
        if (!empty($area_id)) { //区域
            $where .= " and FIND_IN_SET($area_id, area_id)";
        }
        ...
        foreach ($list as $k => $v){
      	...
      	if(!isset($cid)){
          $menu = db::name('class')->where(array('status'=>1,'type'=>3,'pid'=>0))->find();
        }else{
          $menu = db::name('class')->where(array('status'=>1,'type'=>3,'id'=>$cid))->find();
        }
        ...
        $this->assign('classname',custom_another('3'));
        //new add
    ...
    public function show(Request $request){
        ...
        $info = Db::view('novel','id,title,click,good,thumbnail,tag,content,update_time,class,user_id,gold,is_check')
             ->view('member','username,nickname','novel.user_id=member.id','LEFT')
        ...
        $info['username']=!empty($info['nickname'])?$info['nickname']:$info['username'];
