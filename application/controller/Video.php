...
class Video extends BaseController
{
    ...
    public function lists(Request $request)
    {
        ...
        if (!isset($cid)) {
           $menu = db::name('class')->where(array('status'=>1,'type'=>1,'pid'=>0))->find();
        ...
        if(!isset($cid)){
          ...
          $menu = db::name('class')->where(array('status'=>1,'type'=>1,'id'=>$cid))->find();
        }
        ...
        $area_list  = custom_class('1'); //2019/4/22 区域 thirteen 1视频 2为图片 3为资讯
...
//            $where .= " and tag = $tag_id";
		...
		$menu['name'] = empty($menu['name']) ? '视频全部分类' : $menu['name'];
    ...
    public function play(Request $request)
    {
        ...
        if(!$request->isMobile()){
          $seoInfo=get_seo_info_plus();
          $seoInfo['site_title'] = $videoInfo['title'].'_'.$seoInfo['site_title'];
          $this->assign('seo', $seoInfo);
        }
        ...
        //new add 1视频 2为图片 3为资讯
        $this->assign('classname',custom_another('1'));
      	$area_id = empty($videoInfo['area_id']) ? '1': $videoInfo['area_id'];
        $this->assign('classarea',custom_classfind($area_id));
