 ...
 */
/*$arr['message'] = '上传成功';
$arr['url'] = 'http://msvodx.admin.com/static/images/face.jpg';
$arr['state'] = 'ERROR';
echo json_encode($_FILES['undefined']);exit;

echo json_encode($arr, 1);exit;
header('Content:text/html;charset=utf-8');
echo '<script type="text/javascript">window.parent.CKEDITOR.tools.callFunction(1, "http://msvodx.admin.com/static/images/face.jpg", "上传成功啦啦啦");</script> ';EXIT;
...
$newFileName = isset($_REQUEST['newFileName']) ? $_REQUEST['newFileName'] : (time().rand(100000,999999));
// Get a file name
if (isset($_REQUEST["name"])) {
    $fileName = $_REQUEST["name"];
    $fileName=XRenameFileName($fileName,$newFileName);
    $_tmpType=end(explode('.',$fileName));
    if(!in_array($_tmpType,$allowFileType)){
        if($from == 'ueditor'){
            die('{"message":{"code": 102, "message": "Error file  ."},"state":"ERROR"}');
        }else{
            die('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "Error file  ."}, "id" : "id"}');
        }
    }
...
if ($cleanupTargetDir) {
    if (!is_dir($targetDir) || !$dir = opendir($targetDir)) {
        if($from == 'ueditor'){
            die('{"message":{"code": 100, "message": "Failed to open temp directory."},"state":"ERROR"}');
        }else{
...
if (!$out = @fopen("{$filePath}.part", $chunks ? "ab" : "wb")) {
    if($from == 'ueditor'){
        die('{"message":{"code": 102, "message": "Failed to open output stream."},"state":"ERROR"}');
    }else{
...
if (!empty($_FILES)) {
    if ($_FILES["file"]["error"] || !is_uploaded_file($_FILES["file"]["tmp_name"])) {
        if($from == 'ueditor'){
            die('{"message":{"code": 103, "message": "Failed to move uploaded file."},"state":"ERROR"}');
        }else{
    ...
    if (!$in = @fopen($_FILES["file"]["tmp_name"], "rb")) {
        if($from == 'ueditor'){
            die('{"message": {"code": 101, "message": "Failed to open input stream."},"state":"ERROR"}');
        }else{
