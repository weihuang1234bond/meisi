$(function(){
    ...
    $(".select").on("click",function(){
        var $text = $(this).text();
        var class_heiht = parseInt($('#class_heiht').html()); //子分类
        var list_heiht  = parseInt($('#list_heiht').html());  //自定义分类
        if($text == "更多"){
            var self = $(".item label").text();
            if(self == "分类：子分类：标签：排序："){
                $(".vault-top").animate({height:list_heiht+class_heiht+210+"px"});
            }else{
                $(".vault-top").animate({height:list_heiht+class_heiht+165+"px"});
            }
            $(this).text("收起");
        }else{
